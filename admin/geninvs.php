...
session_set_cookie_params(0, '/', '.localhost'); // Change localhost to feds.blog

// Start session
session_start();

// Redirect if username is not set in session
...
if (isset($_POST['geninv'])) {
    ...
    $sql = "INSERT INTO `invites` (code, createdby) VALUES ('$invcode', $uid)";
    $result = mysqli_query($conn, $sql);
}
...
<head>
    ...
    <title>gethaze.lol - dashboard</title>
...
<div class="border">
  <div class="avatar">
      <div class="avatar-container" style="display: flex; justify-content: left; align-items: left;">
          <img width="15px" height="15px" src="<?php echo $avatar;?>" class="inline-effect avatar">
</div>
  </div>

    <a style="font-size: 14px;">Logged in as <a href="https://localhost/<?php echo $username; ?>" style="color: <?php echo $color; ?>;"><?php echo $username; ?></a></a>
  <br>
...
<form action="" method="POST" style="display: flex; justify-content: center; align-items: center; flex-direction: column; gap: 10px;">
    <!-- Use an input field for easy selection of the invite code -->
    <input type="text" value="<?php echo $invcode; ?>" id="inviteCode" readonly 
        style="color: white; background-color: transparent; border: none; margin-top: 10px; padding: 5px; width: auto; text-align: center; cursor: text;">

    <button type="submit" class="btn btn-lg btn-block btn-primary" name="geninv" 
        style="color: white; background-color: transparent; border: 2px solid #ffffff; border-radius: 5px; margin-top: 10px;">
        GENERATE
    </button>
</form>
...
$(document).ready(function() {
    $('#generateInviteForm').submit(function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                // Update the invite code on the page
                $('#inviteCode').html('<b>' + response + '</b>');
  ...
  <?php
if ($hasperms == 1) {
    echo '<a href="../dash/index.php" style="color: white; background-color: transparent; border: none; text-decoration: none;">';
    echo '<button style="color: white; background-color: transparent; border: none;">';
    echo '<h1 style="font-size: 15px;"> Back </h1>';
    echo '</button>';
    echo '</a>';
} else {
    echo '';
